...
public class SerialPortManager extends SerialPort {
    ...
    private ArrayList<OnOpenSerialPortListener> mOnOpenSerialPortListener;
    ...
    private final ReadAndWriteProxy proxy = new ReadAndWriteProxy();
    ...
    public boolean openSerialPort(File device, int baudRate) {
        ...
        try {
            ...
            if (null != mOnOpenSerialPortListener) {
                ...
                ReadAndWriteProxy.getInstance().initFOS(mFileOutputStream);
            }
            // 开启发送消息的线程
            startSendThread();
            // 开启接收消息的线程
    ...
    public SerialPortManager setOnSerialPortDataListener(OnSerialPortDataListener listener) {
        proxy.setOnSerialPortDataListener(listener);
        return this;
    }

    public void removeSerialPortDataListener(OnSerialPortDataListener listener) {
        proxy.removeSerialPortDataListener(listener);
    }
